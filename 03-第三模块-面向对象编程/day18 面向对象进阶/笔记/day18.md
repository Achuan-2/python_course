...
提示：当把每个对象中都存在的**相同的示例变量**时，可以选择把它放在类变量中，这样就可以避免对象中维护多个相同数据。
...
属性其实是由**绑定方法 + 特殊装饰器组合**创造出来的，让我们以后在调用方法时**可以不加括号**，例如：
...
示例：以之前开发的分页的功能。就可以用@property优化
...
其实，除了咱们写的示例以外，在很多模块和框架的源码中也有porperty的身影，例如：requests模块。
...
由于属性和实例变量的调用方式相同，所以在编写时需要注意：**属性名称不要实例变量重名**。
