...
# json格式字符串都用双引号，不用单引号
# 元组也会用中括号包裹[]
```

> 注意：json格式和python的字典还是有区别的
...
- Python数据类型 -> json ，一般称为：序列化，使用json.dumps方法
  ...
  print(res) # '[{"id": 1, "name": "武沛齐", "age": 18}, {"id": 2, "name": "alex", "age": 18}]'
  ```
- json格式 -> Python数据类型，一般称为：反序列化，使用json.loads方法
...
- `json.dump`，将数据序列化并写入文件（不常用，因为用dumps生成json字符串再写入就足够了）
  ...
  file_object.close()
  ```
- `json.load`，读取文件中的数据并反序列化为python的数据类型（不常用，因为用loads输入读取的字符串就足够了）
...
start=time.time()
fun())
duration=time.time()-start #计算函数运行时间
...
- 提取数据区域
  ...
  # 如果不加括号提取的就是15131255789，加了括号相当于我识别出这个区域，再提取括号内匹配的内容
  ```
  ...
  # 如果不加括号提取的就是15131255789，加了两个括号返回一个元组('13', '255789')
  ```

  ```python
  import re

  text = "楼主太牛逼了，在线想要 442662578@qq.com和xxxxx@live.com谢谢楼主，手机号也可15131255789，搞起来呀15131266666"
  data_list = re.findall("15(131(2\d{5}))", text)
  print(data_list) #[('131255789', '255789'), ('131266666', '266666')]
  # 这里用的是嵌套的括号，但貌似返回的元组没有嵌套关系，还是和多个括号一样，是平行的展示
  ```
- 获取指定区域 + 或条件
...
1. 利用正则匹配QQ号码
   ...
   [1-9]\d{4,}
   ```
2. 身份证号码：一共18位，倒数第二位奇数代表男，偶数代表女，最后一位是校验码，可能是数字或X
...
├── db/            文件夹，存放数据
├── files/         文件夹，存放文件
├── src/           包，业务处理的代码，主要跑的代码
└── utils/         包，公共功能,比如发邮件的模块等
```
...
自动采集梨视频1000条资讯的爬虫脚本。`<span style='color:red;'>`梨视频平台系统更新后可能会导致下载失败，到时候需根据平台调整再来修改代码。
